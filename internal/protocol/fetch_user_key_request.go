...
// FetchUserKeyRequest represents a request to retrieve the encrypted master key for a username.
...
func (f *FetchUserKeyRequest) MarshalBinary() ([]byte, error) {
	if f.Username == nil {
		return nil, errors.New("protocol: FetchUserKeyRequest Username cannot be nil")
	}
